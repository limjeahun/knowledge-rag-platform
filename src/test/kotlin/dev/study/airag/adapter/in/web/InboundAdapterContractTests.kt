...
class InboundAdapterContractTests {
    ...
    fun `controller translates every HTTP contract through inbound ports`() {
        ...
        val registered =
            controller.register(RegisterKnowledgeDocumentRequest("RAG", "content", mapOf("team" to "ai")))
        val document = controller.get("document-1")
        val retried = controller.retry("document-1")
        controller.delete("document-1")
        val searched = controller.search("query", 3, 0.7)
        val answered = controller.chat(AskKnowledgeRequest("question", 4, 0.8))
    ...
    fun `MCP tools translate tool parameters through the same inbound ports`() {
        ...
        assertEquals(listOf(hit), tools.search("query", 7))
