...
class KnowledgeApplicationServiceTests {
    ...
    @Test
    fun `search rejects blank text and values outside the query contract`() {
        val service = queryService()
        val invalidQueries =
            listOf(
                SearchKnowledgeQuery(" ", 5, 0.5) to "검색어나 질문은 비어 있을 수 없습니다.",
                SearchKnowledgeQuery("rag", 0, 0.5) to "topK는 1 이상 20 이하이어야 합니다.",
                SearchKnowledgeQuery("rag", 21, 0.5) to "topK는 1 이상 20 이하이어야 합니다.",
                SearchKnowledgeQuery("rag", 5, -0.1) to "similarityThreshold는 0.0 이상 1.0 이하이어야 합니다.",
                SearchKnowledgeQuery("rag", 5, 1.1) to "similarityThreshold는 0.0 이상 1.0 이하이어야 합니다.",
                SearchKnowledgeQuery("rag", 5, Double.NaN) to
                    "similarityThreshold는 0.0 이상 1.0 이하이어야 합니다.",
            )

        invalidQueries.forEach { (query, expectedMessage) ->
            val exception = assertFailsWith<IllegalArgumentException> { service.search(query) }
